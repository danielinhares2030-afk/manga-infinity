        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 relative -mt-32 md:-mt-40 z-10">  
            <div className="flex flex-col md:flex-row gap-8 items-center md:items-start">  
                <div className="w-44 md:w-56 flex-shrink-0 relative group animate-[levitar_6s_ease-in-out_infinite]">  
                    ...
                    <div className="absolute -inset-2 bg-gradient-to-b from-cyan-500/30 to-red-500/30 rounded-2xl blur-lg opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none"></div>  
                        ...
                        <div className="relative z-50">  
                            ...
                            {showLibraryMenu && (  
                                <div className="absolute top-full mt-2 left-1/2 -translate-x-1/2 md:left-0 md:-translate-x-0 w-48 bg-[#08080a] border border-cyan-900/50 rounded-2xl p-2 shadow-[0_20px_40px_rgba(0,0,0,0.9)] animate-in fade-in zoom-in-95">  
                                    ...
                                    {inLibrary && <button onClick={() => updateLibraryStatus('Remover')} className="w-full text-left px-4 py-3 rounded-xl text-xs font-black uppercase tracking-widest text-red-400 hover:bg-red-950 hover:text-red-300 transition-colors mt-1 border-t border-white/5">Remover Obra</button>}  
                                </div>  
                            )}  
                        </div>  
            ...
            <div className="mt-10 flex flex-wrap justify-center md:justify-start gap-2 relative z-10">  
                {manga.genres?.map(genre => (  
                    <span key={genre} className="bg-white/[0.02] border border-white/5 text-cyan-100/80 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] cursor-default hover:border-cyan-500 hover:text-cyan-300 transition-colors">{genre}</span>  
                ))}  
            </div>  
                                ...
                                return (  
                                    <div key={chapter.id} onClick={() => onChapterClick(manga, chapter)} className={`cursor-pointer p-4 rounded-2xl flex items-center justify-between transition-all duration-300 border shadow-sm group ${isRead ? 'bg-black/40 border-transparent opacity-60 hover:bg-white/[0.02]' : 'bg-white/[0.02] border-white/5 hover:border-cyan-500/50 hover:shadow-[0_0_15px_rgba(34,211,238,0.2)] hover:bg-white/[0.04]'}`}>  
                                        <div className="flex items-center gap-4">  
                                            <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-black text-sm transition-colors ${isRead ? 'bg-white/5 text-gray-600' : 'bg-[#08080a] text-cyan-400 border border-cyan-500/30 group-hover:bg-cyan-500 group-hover:text-black'}`}>{chapter.number}</div>  
                                            <div>  
