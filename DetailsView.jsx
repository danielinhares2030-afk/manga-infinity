...
import { ArrowLeft, Star, Play, Library, Share2, BookOpen, CheckCircle, Clock, Sparkles, MessageSquare, ChevronDown } from 'lucide-react';
...
export default function DetailsView({ manga, libraryData, historyData, user, userProfileData, onBack, onChapterClick, onRequireLogin, showToast, db }) {
    ...
    const [detailsTab, setDetailsTab] = useState('capitulos'); // Controla abas (Capítulos / Comentários)
    const [showLibraryMenu, setShowLibraryMenu] = useState(false); // Controla menu suspenso da biblioteca
    ...
    return (
        <div className="min-h-screen bg-[#020105] text-gray-200 font-sans pb-24 animate-in fade-in duration-700 relative overflow-hidden">
            ...
            <div className="absolute top-1/3 right-1/4 w-[300px] h-[300px] bg-fuchsia-600/10 rounded-full blur-[100px] pointer-events-none mix-blend-screen animate-[pulse_8s_ease-in-out_infinite_reverse]"></div>
            ...
            <div className="relative h-64 md:h-80 w-full overflow-hidden border-b border-white/5">
                <div className="absolute inset-0 bg-[#020105]">
                    <img src={manga.coverUrl} className="w-full h-full object-cover opacity-30 blur-xl scale-110" alt="Capa de Fundo" />
                </div>
                <div className="absolute inset-0 bg-gradient-to-t from-[#020105] via-[#020105]/80 to-transparent" />
            ...
            <div className="max-w-5xl mx-auto px-4 sm:px-6 relative -mt-32 md:-mt-40 z-10">
                <div className="flex flex-col md:flex-row gap-8 items-center md:items-start">
                    <div className="w-44 md:w-56 flex-shrink-0 relative group animate-[levitar_6s_ease-in-out_infinite]">
                        ...
                        <div className="absolute -inset-2 bg-gradient-to-b from-cyan-500/20 to-fuchsia-500/20 rounded-2xl blur-lg opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none"></div>
                    ...
                    <div className="flex-1 text-center md:text-left mt-2 md:mt-12 relative z-10">
                        ...
                        <div className="flex flex-wrap items-center justify-center md:justify-start gap-3">
                            <button onClick={() => nextChapterToRead ? onChapterClick(manga, nextChapterToRead) : showToast("Nenhum capítulo", "warning")} className="flex-1 md:flex-none bg-white text-black hover:bg-transparent hover:text-white border border-transparent hover:border-white font-black px-8 py-4 rounded-2xl transition-all duration-300 text-xs uppercase tracking-widest flex items-center justify-center gap-2 shadow-[0_0_20px_rgba(255,255,255,0.2)]">
                                <Play className="w-4 h-4 fill-current" /> {lastRead ? 'Continuar Leitura' : 'Iniciar Leitura'}
                            </button>
                            
                            {/* BOTÃO DA BIBLIOTECA COM DROPDOWN */}
                            <div className="relative">
                                <button onClick={() => setShowLibraryMenu(!showLibraryMenu)} className={`px-5 py-4 rounded-2xl border flex items-center justify-center transition-all duration-300 shadow-sm gap-2 font-black text-[10px] uppercase tracking-widest ${inLibrary ? 'bg-cyan-500/10 border-cyan-500/50 text-cyan-400 shadow-[0_0_15px_rgba(34,211,238,0.2)]' : 'bg-white/5 border-white/10 text-gray-400 hover:text-white hover:bg-white/10 hover:border-cyan-400'}`}>
                                ...
                                {showLibraryMenu && (
                                    <div className="absolute top-full mt-2 left-1/2 -translate-x-1/2 md:left-0 md:-translate-x-0 w-48 bg-[#05030a] border border-cyan-900/50 rounded-2xl p-2 z-50 shadow-[0_20px_40px_rgba(0,0,0,0.8)] animate-in fade-in zoom-in-95">
                                        {libraryStatuses.map(status => (
                                            <button key={status} onClick={() => updateLibraryStatus(status)} className="w-full text-left px-4 py-3 rounded-xl text-xs font-black uppercase tracking-widest text-gray-300 hover:bg-cyan-900/40 hover:text-cyan-400 transition-colors">
                                                {status}
                                            </button>
                                        ))}
                                        {inLibrary && (
                                            <button onClick={() => updateLibraryStatus('Remover')} className="w-full text-left px-4 py-3 rounded-xl text-xs font-black uppercase tracking-widest text-red-400 hover:bg-red-950 hover:text-red-300 transition-colors mt-1 border-t border-white/5">
                                                Remover Obra
                                            </button>
                                        )}
                                    </div>
                ...
                <div className="mt-10 flex flex-wrap justify-center md:justify-start gap-2 relative z-10">
                    {manga.genres?.map(genre => (
                        <span key={genre} className="bg-white/[0.02] border border-white/5 text-gray-300 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] cursor-default hover:bg-white/5 transition-colors">{genre}</span>
                    ))}
                </div>

                <div className="mt-8 relative z-10">
                    <div className="bg-white/[0.02] backdrop-blur-md border border-white/5 p-6 md:p-8 rounded-[2rem] shadow-inner">
                        <h3 className="text-white font-black mb-4 text-xs uppercase tracking-widest flex items-center gap-2"><Sparkles className="w-4 h-4 text-cyan-400" /> Sinopse</h3>
                ...
                <div className="mt-12 relative z-10">
                    {/* NAVEGAÇÃO ENTRE CAPÍTULOS E COMENTÁRIOS */}
                    <div className="flex items-center gap-4 mb-8 border-b border-white/5 pb-4">
                        <button onClick={() => setDetailsTab('capitulos')} className={`text-xs font-black uppercase tracking-[0.2em] transition-colors pb-2 border-b-2 ${detailsTab === 'capitulos' ? 'text-cyan-400 border-cyan-400' : 'text-gray-500 border-transparent hover:text-gray-300'}`}>
                            Capítulos ({chapters.length})
                        </button>
                        <button onClick={() => setDetailsTab('comentarios')} className={`text-xs font-black uppercase tracking-[0.2em] transition-colors pb-2 border-b-2 ${detailsTab === 'comentarios' ? 'text-cyan-400 border-cyan-400' : 'text-gray-500 border-transparent hover:text-gray-300'}`}>
                            Comentários
                        </button>
                    </div>
                                    ...
                                    return (
                                        <div key={chapter.id} onClick={() => onChapterClick(manga, chapter)} className={`cursor-pointer p-4 rounded-2xl flex items-center justify-between transition-all duration-300 border shadow-sm group ${isRead ? 'bg-black/40 border-transparent opacity-60 hover:bg-white/[0.02]' : 'bg-white/[0.02] border-white/5 hover:border-cyan-500/50 hover:shadow-[0_0_15px_rgba(34,211,238,0.2)] hover:bg-white/[0.04]'}`}>
                                            <div className="flex items-center gap-4">
                                                <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-black text-sm transition-colors ${isRead ? 'bg-white/5 text-gray-600' : 'bg-[#05030a] text-cyan-400 border border-cyan-500/30 group-hover:bg-cyan-500 group-hover:text-black'}`}>{chapter.number}</div>
                                                <div>
