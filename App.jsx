...
function MangaInfinityApp() {
  ...
  useEffect(() => { const timer = setTimeout(() => setSplashTimerDone(true), 3500); return () => clearTimeout(timer); }, []);
  ...
  return (
    <div className={`min-h-screen font-sans selection:bg-cyan-600 selection:text-white flex flex-col transition-colors duration-300 ${getThemeClasses(userSettings.theme)}`}>
      ...
      {levelUpAlert && ( <div className="fixed top-20 right-4 z-[99999] bg-[#0d0d12]/95 backdrop-blur-md border border-white/10 shadow-[0_5px_30px_rgba(217,70,239,0.2)] text-white px-4 py-3 rounded-lg flex items-center gap-3 animate-in slide-in-from-right fade-out duration-300 pointer-events-none"><div className="bg-gradient-to-br from-cyan-600 to-fuchsia-600 p-2 rounded-md shadow-inner"><Trophy className="w-5 h-5 text-white" /></div><div className="flex flex-col"><span className="text-[10px] text-cyan-400 uppercase tracking-widest font-black">Level Up!</span><span className="text-sm font-bold">Nível {levelUpAlert} Alcançado</span></div></div> )}
      {dropAlert && ( <div className="fixed bottom-24 right-4 z-[99999] bg-[#0d0d12]/90 backdrop-blur-md border border-cyan-500/50 shadow-[0_0_20px_rgba(34,211,238,0.3)] px-3 py-2 rounded-lg flex items-center gap-2 animate-in slide-in-from-bottom-5 fade-out duration-300 pointer-events-none"><Hexagon className="w-4 h-4 text-cyan-400 animate-pulse" /><span className="text-cyan-100 text-xs font-bold">+1 Cristal</span></div> )}
      {isRandomizing && ( <div className="fixed inset-0 z-[2000] bg-[#050508]/90 backdrop-blur-sm flex flex-col items-center justify-center animate-in fade-in duration-100 pointer-events-none"><Dices className="w-24 h-24 text-fuchsia-500 animate-[spin_0.2s_linear_infinite] drop-shadow-[0_0_50px_rgba(217,70,239,0.6)]" /></div> )}
                  ...
                  <div className="cursor-pointer flex items-center gap-3 group" onClick={() => navigateTo('profile')}>
                    ...
                    <div className={`relative w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 group ${(!eq.moldura?.preview && eq.moldura) ? eq.moldura.cssClass : ''}`}>
                        <div className={`w-9 h-9 rounded-full overflow-hidden bg-[#161a25] flex items-center justify-center relative z-10 ${!eq.moldura ? 'border border-white/10 group-hover:border-cyan-500' : ''}`}>
                            <img src={activeAvatarSrc} className={`w-full h-full object-cover transition-colors duration-300 ${eq.avatar?.cssClass || ''}`} alt="Avatar" onError={(e) => e.target.src = `https://placehold.co/100x100/0f111a/3b82f6?text=U`} />
                        </div>
                        {cleanCosmeticUrl(eq.particulas?.preview) && <img src={cleanCosmeticUrl(eq.particulas.preview)} onError={(e)=>e.target.style.display='none'} className={`absolute inset-[-50%] m-auto w-[200%] h-[200%] object-contain z-10 ${eq.particulas.cssClass || ''}`} style={{ mixBlendMode: 'screen', WebkitMixBlendMode: 'screen', pointerEvents: 'none' }} />}
                        ...
                        {cleanCosmeticUrl(eq.badge?.preview) && <img src={cleanCosmeticUrl(eq.badge.preview)} onError={(e)=>e.target.style.display='none'} className={`absolute -bottom-1 -right-1 w-4 h-4 object-contain z-40 ${eq.badge.cssClass || ''}`} style={{ pointerEvents: 'none' }} />}
                    </div>
                  </div>
