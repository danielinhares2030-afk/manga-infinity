...
// O "export" aqui é obrigatório para o Vercel não dar erro no App.jsx!
export function CatalogView({ mangas, onNavigate, dataSaver, catalogState, setCatalogState }) {
    ...
    return (
        <div className="pb-32 min-h-screen relative font-sans text-white bg-[#050505] overflow-x-hidden selection:bg-red-600/30">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 relative z-10">
                ...
                <div className="relative mb-6">
                    ...
                    <input 
                        ...
                        className="w-full pl-12 pr-4 py-4 rounded-xl bg-[#111] border border-white/5 text-white outline-none focus:border-red-600 transition-colors font-medium text-sm placeholder:text-gray-600 shadow-inner" 
                        placeholder="Pesquisar obras, autores..." 
                    />
                </div>

                {/* PAINEL DE FILTROS */}
                <div className="bg-[#0A0A0A] border border-white/5 rounded-2xl p-5 mb-8 shadow-lg">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        ...
                        <div className="flex flex-col gap-1.5">
                            ...
                            <select value={selectedType} onChange={e=>setSelectedType(e.target.value)} className="w-full bg-[#111] border border-white/5 text-gray-200 text-xs font-bold rounded-lg px-3 py-3 outline-none cursor-pointer focus:border-red-600 transition-colors">
                        ...
                        <div className="flex flex-col gap-1.5">
                            ...
                            <select value={selectedGenre} onChange={e=>setSelectedGenre(e.target.value)} className="w-full bg-[#111] border border-white/5 text-gray-200 text-xs font-bold rounded-lg px-3 py-3 outline-none cursor-pointer focus:border-red-600 transition-colors">
                        ...
                        <div className="flex flex-col gap-1.5">
                            ...
                            <select value={selectedStatus} onChange={e=>setSelectedStatus(e.target.value)} className="w-full bg-[#111] border border-white/5 text-gray-200 text-xs font-bold rounded-lg px-3 py-3 outline-none cursor-pointer focus:border-red-600 transition-colors">
                        ...
                        <div className="flex flex-col gap-1.5">
                            ...
                            <select value={sortBy} onChange={e=>setSortBy(e.target.value)} className="w-full bg-[#111] border border-white/5 text-gray-200 text-xs font-bold rounded-lg px-3 py-3 outline-none cursor-pointer focus:border-red-600 transition-colors">
                ...
                <div className={viewMode === 'grid' ? "grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 md:gap-5" : "flex flex-col gap-3"}>
                    {currentItems.length > 0 ? currentItems.map(manga => (
                        <div key={manga.id} onClick={() => onNavigate('details', manga)} className={`cursor-pointer group relative bg-[#0A0A0A] rounded-xl overflow-hidden transition-all duration-300 hover:ring-1 hover:ring-red-600/50 shadow-lg ${viewMode === 'list' ? 'flex flex-row items-center h-32 pr-4 border border-white/5' : 'flex flex-col aspect-[2/3]'}`}>
                            
                            <div className={`relative bg-[#111] ${viewMode === 'grid' ? 'absolute inset-0 w-full h-full' : 'w-24 h-full flex-shrink-0'}`}>
                                <img src={manga.coverUrl} className="w-full h-full object-cover opacity-90 group-hover:opacity-100 group-hover:scale-105 transition-all duration-500" loading="lazy" onError={(e) => e.target.src = `https://placehold.co/300x450/111111/444444?text=Obra`} />
                                {viewMode === 'grid' && <div className="absolute inset-0 bg-gradient-to-t from-[#050505] via-black/40 to-transparent"></div>}
                            </div>
                            
                            {viewMode === 'grid' && (
                                <>
                                    {/* BADGE ORIGEM: FINO E ELEGANTE */}
                            ...
                            {viewMode === 'list' && (
                                <div className="flex-1 flex flex-col justify-center pl-4 min-w-0">
                                    ...
                                    <h3 className="text-sm font-bold text-white mb-1 line-clamp-1 group-hover:text-red-500 transition-colors">{manga.title}</h3>
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="text-[10px] font-bold text-red-500 border-l border-white/10 pl-3">
                                            Cap. {manga.chapters?.length ? manga.chapters[0].number : 0}
                                        </span>
                                    </div>
                                </div>
                ...
                {totalPages > 1 && (
                    <div className="mt-12 flex items-center justify-center gap-2">
                        <button disabled={currentPage === 1} onClick={() => handlePageChange(currentPage - 1)} className="p-3 rounded-lg bg-[#0A0A0A] text-gray-400 border border-white/5 disabled:opacity-30 hover:bg-[#222] hover:text-white transition-colors">
                        ...
                        <div className="flex items-center gap-1 bg-[#0A0A0A] p-1 rounded-lg border border-white/5">
                            {[...Array(totalPages)].map((_, i) => {
                                ...
                                return (
                                    <button key={i} onClick={() => handlePageChange(i + 1)} className={`w-10 h-10 rounded-md font-bold text-sm transition-colors ${currentPage === i + 1 ? 'bg-red-600 text-white' : 'bg-transparent text-gray-500 hover:bg-[#222] hover:text-white'}`}>
                                        {i + 1}
                                    </button>
                                );
                            })}
                        ...
                        <button disabled={currentPage === totalPages} onClick={() => handlePageChange(currentPage + 1)} className="p-3 rounded-lg bg-[#0A0A0A] text-gray-400 border border-white/5 disabled:opacity-30 hover:bg-[#222] hover:text-white transition-colors">
