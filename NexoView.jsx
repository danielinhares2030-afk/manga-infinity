...
import { Target, Hexagon, Trophy, Timer, Skull, Zap, Loader2, ArrowRight, Key, Sparkles, Flame, AlertTriangle, Crown, ChevronDown, Globe, ChevronRight, User, Image, Circle, Calendar, BookOpen, EyeOff, X } from 'lucide-react';
...
export function NexoView({ user, userProfileData, showToast, mangas, onNavigate, onLevelUp, synthesizeCrystal, shopItems }) {
    ...
    return (
        <div className={`pb-24 animate-in fade-in duration-500 relative font-sans min-h-screen text-gray-200 ${equipped.tema_perfil ? equipped.tema_perfil.cssClass : 'bg-[#030305]'}`}>
            ...
            {/* MODAL CUSTOMIZADO: SACRIFÍCIO VITAL */}
            {showSacrificeModal && (
                <div className="fixed inset-0 z-[99999] bg-black/90 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in zoom-in-95 duration-200">
                    <div className="bg-[#050505] border border-red-600/60 rounded-2xl p-8 max-w-sm w-full shadow-[0_0_50px_rgba(220,38,38,0.2)] text-center relative overflow-hidden">
                        ...
                        <Zap className="w-14 h-14 text-red-500 mx-auto mb-5 relative z-10 drop-shadow-[0_0_15px_rgba(220,38,38,0.8)]" />
                        <h3 className="text-xl font-black text-white uppercase tracking-widest mb-3 relative z-10">Sacrifício Vital</h3>
                        <p className="text-gray-400 text-xs font-bold mb-8 relative z-10 leading-relaxed">Você transmutará <span className="text-red-500">1000 XP</span> do seu próprio sangue. Seu nível pode retroceder. Deseja forjar a caixa mesmo assim?</p>
                        <div className="flex gap-4 relative z-10">
                            <button onClick={() => setShowSacrificeModal(false)} className="flex-1 bg-[#0a0a0c] border border-white/10 text-gray-300 py-3.5 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-white/5 transition-colors">Recuar</button>
                            <button onClick={executeBuyBoxWithXP} className="flex-1 bg-red-600 hover:bg-red-500 text-white py-3.5 rounded-xl font-black text-[10px] uppercase tracking-widest shadow-[0_0_20px_rgba(220,38,38,0.4)] transition-all">Sacrificar XP</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {isOpeningBoxAnim && (
                <div className="fixed inset-0 z-[9999] bg-[#000000] flex flex-col items-center justify-center overflow-hidden">
                    ...
                    <div className="absolute bottom-1/4 left-0 w-full flex justify-center z-40 pointer-events-none">
                        <h2 className="text-2xl md:text-3xl font-black text-white uppercase tracking-[0.4em] drop-shadow-[0_0_10px_rgba(220,38,38,0.8)] text-hide-after absolute">Rompendo Selo...</h2>
                        <h2 className="text-3xl md:text-4xl font-black text-red-500 uppercase tracking-[0.5em] drop-shadow-[0_0_20px_rgba(220,38,38,1)] text-show-after absolute">Revelando!</h2>
                    </div>
                </div>
            )}
                        ...
                        <div className="grid md:grid-cols-2 gap-6">
                            ...
                            <div className="flex flex-col gap-6">
                                <div className="bg-[#050505] border border-white/5 rounded-2xl p-6 flex flex-col justify-center relative overflow-hidden shadow-lg">
                                    ...
                                    {canClaimDaily ? (
                                        <button onClick={handleClaimDaily} className="bg-transparent border border-red-500 text-red-500 hover:bg-red-600 hover:text-white font-black py-3 rounded-xl text-[10px] uppercase tracking-widest transition-all">Resgatar Oferenda</button>
                                    ) : (
                                        <button disabled className="bg-[#0a0a0c] border border-white/5 text-gray-600 font-black py-3 rounded-xl text-[10px] uppercase tracking-widest cursor-not-allowed">Já Resgatado Hoje</button>
                                    )}
                                </div>
                    ...
                    <div className="animate-in fade-in duration-300 max-w-2xl mx-auto mt-4 relative z-10">
                        <div className="bg-[#050505] border border-red-600/30 p-10 md:p-14 text-center relative overflow-hidden shadow-[0_0_50px_rgba(220,38,38,0.15)] rounded-tl-3xl rounded-br-3xl">
                            ...
                            <button onClick={runSynthesis} disabled={synthesizing || (userProfileData.crystals || 0) < 5} className="w-full bg-red-600 text-white hover:bg-red-500 disabled:bg-[#0a0a0c] disabled:border border-white/5 disabled:text-gray-600 font-black py-5 transition-all duration-300 relative z-10 text-xs uppercase tracking-widest flex items-center justify-center gap-3 shadow-[0_0_20px_rgba(220,38,38,0.4)] disabled:shadow-none transform skew-x-[-10deg]">
                                <div className="skew-x-[10deg] flex items-center gap-2">{synthesizing ? <><Loader2 className="w-5 h-5 animate-spin" /> Transmutando Matéria...</> : 'Iniciar Transmutação'}</div>
                            </button>
                        </div>
                    </div>
                )}
                ...
                {activeTab === "Ranking" && (
                    <div className="animate-in fade-in duration-300 max-w-4xl mx-auto pb-10">
                        <div className="flex justify-between items-end mb-10 border-b border-red-900/30 pb-4">
                            <div>
                                <h2 className="text-3xl md:text-4xl font-black text-white uppercase tracking-tighter flex items-center gap-3"><span className="text-red-600"><svg className="w-8 h-8 fill-current" viewBox="0 0 24 24"><path d="M12 2L14.5 9.5L22 12L14.5 14.5L12 22L9.5 14.5L2 12L9.5 9.5L12 2Z"/></svg></span>RANKING DE LEITORES</h2>
                            ...
                            <>
                                <div className="flex justify-center items-end gap-2 md:gap-4 mb-10 mt-8">
                                    {rankingList[1] && (
                                        <div onClick={() => setSelectedUserId(rankingList[1].id)} className="w-[30%] max-w-[160px] bg-gradient-to-b from-[#1a1a1a] to-[#050505] rounded-t-2xl border-t-[3px] border-gray-400 p-4 relative flex flex-col items-center shadow-[0_-10px_30px_rgba(156,163,175,0.15)] order-1 cursor-pointer hover:border-white transition-colors group">
                                            ...
                                            <h4 className="font-black text-xs text-white uppercase truncate w-full text-center group-hover:text-red-400">{rankingList[1].displayName || "Oculto"}</h4>
                                ...
                                <div className="flex flex-col gap-3">
                                    {rankingList.slice(3).map((player, idx) => (
                                        <div key={player.id} onClick={() => setSelectedUserId(player.id)} className="bg-[#050505] border border-red-900/30 hover:border-red-600/50 rounded-2xl p-4 flex items-center gap-4 shadow-lg transition-colors group cursor-pointer">
                                            ...
                                            <div className="flex-1 min-w-0 flex flex-col justify-center">
                                                <h4 className="text-white font-black text-sm uppercase truncate group-hover:text-red-400 transition-colors">{player.displayName || "Oculto"}</h4>
