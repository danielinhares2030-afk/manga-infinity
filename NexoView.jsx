        ...
        <div className={`max-w-4xl mx-auto px-4 py-6 animate-in fade-in duration-500 relative pb-24 font-sans text-gray-300 min-h-screen ${equipped.tema_perfil ? equipped.tema_perfil.cssClass : 'bg-[#020205]'}`}>
            ...
            {activeTab === "Forja" && (
                <div className="animate-in fade-in duration-500">
                    <div className="bg-gradient-to-b from-[#050508] to-black border border-amber-900/20 p-6 sm:p-10 rounded-3xl text-center shadow-2xl relative overflow-hidden">
                        ...
                        <div className="bg-[#020205] border border-white/5 rounded-2xl p-6 max-w-xs mx-auto mb-8 shadow-inner relative z-10">
                    ...
                    <div className="bg-[#020205] border border-blue-900/20 p-6 md:p-8 rounded-3xl shadow-xl relative overflow-hidden">
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 sm:gap-5 relative z-10">
                        {shopItems.map(item => {
                          ...
                          // VERIFICAÇÃO TOLOWERCASE PARA EVITAR QUEBRA NO TIPO
                          ...
                          return (
                            <div key={item.id} className={`bg-black border p-5 rounded-2xl flex flex-col items-center text-center transition-all duration-300 shadow-inner group ${isEquipped ? 'border-amber-500/40 shadow-[0_0_20px_rgba(245,158,11,0.15)]' : 'border-blue-900/20 hover:border-amber-900/50 hover:shadow-[0_0_15px_rgba(37,99,235,0.1)]'}`}>
                              ...
                              {/* ESTRUTURA DOS CARD DA LOJA COM MIX-BLEND-SCREEN */}
                              <div className={`w-20 h-20 rounded-xl mb-4 bg-[#050508] flex items-center justify-center overflow-hidden shadow-inner border border-white/5 relative flex-shrink-0 group ${(!item.preview && ['moldura', 'efeito'].includes(cat)) ? item.cssClass : ''}`}>
                                ...
                                {cat === 'particulas' && item.preview && <img src={item.preview} className={`absolute inset-[-50%] m-auto w-[200%] h-[200%] object-contain z-10 ${item.cssClass}`} style={{ mixBlendMode: 'screen', WebkitMixBlendMode: 'screen', pointerEvents: 'none' }} />}
                                ...
                                {['moldura', 'efeito', 'particulas', 'badge'].includes(cat) && (
                                    <div className="w-12 h-12 rounded-full flex items-center justify-center relative z-0">
                                        <User className="w-8 h-8 text-gray-600 bg-[#0d0d12] rounded-full p-1 w-full h-full object-cover" />
                                    </div>
                                )}

                                {cat === 'avatar' && item.preview && <img src={item.preview} className={`w-full h-full object-cover group-hover:scale-110 transition-transform duration-500 ${item.cssClass}`} />}
                                
                                {cat === 'efeito' && item.preview && <img src={item.preview} className={`absolute inset-0 m-auto w-full h-full object-contain z-10 ${item.cssClass}`} style={{ mixBlendMode: 'screen', WebkitMixBlendMode: 'screen', pointerEvents: 'none' }} />}
                                {cat === 'moldura' && item.preview && <img src={item.preview} className={`absolute inset-[-15%] m-auto w-[130%] h-[130%] object-contain z-10 ${item.cssClass}`} style={{ mixBlendMode: 'screen', WebkitMixBlendMode: 'screen', pointerEvents: 'none' }} />}
                                {cat === 'badge' && item.preview && <img src={item.preview} className={`absolute -bottom-1 -right-1 w-6 h-6 object-contain z-20 ${item.cssClass}`} style={{ pointerEvents: 'none' }} />}
                            ...
                            <div className="space-y-3 relative z-10">
                                {rankingList.map((player, index) => (
                                    <div key={player.id} className={`bg-black border p-4 rounded-2xl flex items-center gap-4 transition-all duration-300 hover:scale-[1.02] ${index < 3 ? 'border-amber-500/40 shadow-[0_0_20px_rgba(245,158,11,0.1)]' : 'border-blue-900/30 hover:border-blue-500/50'}`}>
                                        ...
                                        {/* AVATAR DO RANKING CORRIGIDO COM MIX-BLEND */}
                                        <div className={`relative w-12 h-12 rounded-full flex items-center justify-center flex-shrink-0 group ${(!player.equipped_items?.moldura?.preview && player.equipped_items?.moldura) ? player.equipped_items.moldura.cssClass : ''}`}>
                                            <div className="w-10 h-10 rounded-full overflow-hidden border border-white/10 relative z-10 bg-[#0d0d12]">
                                                <img src={player.avatarUrl || `https://placehold.co/100x100/050508/3b82f6?text=${player.displayName?.charAt(0) || 'U'}`} className="w-full h-full object-cover z-0" />
                                            </div>
                                            {player.equipped_items?.particulas?.preview && <img src={player.equipped_items.particulas.preview} className={`absolute inset-[-50%] m-auto w-[200%] h-[200%] object-contain z-10 ${player.equipped_items.particulas.cssClass}`} style={{ mixBlendMode: 'screen', WebkitMixBlendMode: 'screen', pointerEvents: 'none' }} />}
                                            {player.equipped_items?.efeito?.preview && <img src={player.equipped_items.efeito.preview} className={`absolute inset-0 m-auto w-full h-full object-contain z-20 ${player.equipped_items.efeito.cssClass}`} style={{ mixBlendMode: 'screen', WebkitMixBlendMode: 'screen', pointerEvents: 'none' }} />}
                                            {player.equipped_items?.moldura?.preview && <img src={player.equipped_items.moldura.preview} className={`absolute inset-[-15%] m-auto w-[130%] h-[130%] object-contain z-30 ${player.equipped_items.moldura.cssClass}`} style={{ mixBlendMode: 'screen', WebkitMixBlendMode: 'screen', pointerEvents: 'none' }} />}
                                        </div>
