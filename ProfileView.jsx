...
export function ProfileView({ user, userProfileData, historyData, libraryData, dataLoaded, userSettings, updateSettings, onLogout, onUpdateData, showToast, mangas, onNavigate }) {
  ...
  return (
    <div className={`animate-in fade-in duration-500 w-full pb-20 font-sans min-h-screen text-gray-300 ${eq.tema_perfil ? eq.tema_perfil.cssClass : 'bg-[#0f111a]'}`}>
      {confirmAction && (
      ...
      <div className="h-40 md:h-64 w-full bg-[#0f111a] relative group border-b border-blue-900/20 overflow-hidden">
        {eq.capa_fundo?.preview ? (
            <img src={eq.capa_fundo.preview} onError={(e) => e.target.style.display = 'none'} className={`w-full h-full object-cover opacity-90 ${eq.capa_fundo.cssClass || ''}`} />
        ) : coverBase64 ? (
            <img src={coverBase64} className="w-full h-full object-cover opacity-70 mix-blend-luminosity" /> 
        ) : (
            <div className={`w-full h-full bg-gradient-to-tr from-[#0f111a] to-[#1e2333] ${eq.capa_fundo?.cssClass || ''}`} />
        )}
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 relative -mt-16 md:-mt-20 z-10">
        <div className="flex flex-col md:flex-row items-center md:items-end gap-4 mb-8">
          ...
          {/* AVATAR DO PERFIL BLINDADO CONTRA IMAGENS QUEBRADAS */}
          <div className={`relative w-28 h-28 md:w-36 md:h-36 rounded-full flex items-center justify-center flex-shrink-0 group ${(!eq.moldura?.preview && eq.moldura) ? eq.moldura.cssClass : ''}`}>
            ...
            <div className={`w-full h-full rounded-full bg-[#161a25] flex items-center justify-center relative z-10 overflow-hidden ${!eq.moldura ? 'border-4 border-[#0f111a] shadow-[0_0_30px_rgba(37,99,235,0.15)]' : ''}`}>
               <img 
                 src={eq.avatar?.preview || avatarBase64 || `https://placehold.co/150x150/0f111a/3b82f6?text=U`} 
                 className={`w-full h-full object-cover ${eq.avatar?.cssClass || ''}`} 
                 alt="Avatar"
                 onError={(e) => e.target.src = `https://placehold.co/150x150/0f111a/3b82f6?text=U`}
               />
            </div>
            
            {eq.particulas?.preview && (
              <img src={eq.particulas.preview} onError={(e) => e.target.style.display = 'none'} className={`absolute inset-[-50%] m-auto w-[200%] h-[200%] object-contain z-0 ${eq.particulas.cssClass || ''}`} style={{ mixBlendMode: 'screen', WebkitMixBlendMode: 'screen', pointerEvents: 'none' }} />
            )}
            
            {eq.efeito?.preview && (
              <img src={eq.efeito.preview} onError={(e) => e.target.style.display = 'none'} className={`absolute inset-0 m-auto w-full h-full object-contain z-20 ${eq.efeito.cssClass || ''}`} style={{ mixBlendMode: 'screen', WebkitMixBlendMode: 'screen', pointerEvents: 'none' }} />
            )}

            {eq.moldura?.preview && (
              <img src={eq.moldura.preview} onError={(e) => e.target.style.display = 'none'} className={`absolute inset-[-15%] m-auto w-[130%] h-[130%] object-contain z-30 ${eq.moldura.cssClass || ''}`} style={{ mixBlendMode: 'screen', WebkitMixBlendMode: 'screen', pointerEvents: 'none' }} />
            )}

            {eq.badge?.preview && (
              <img src={eq.badge.preview} onError={(e) => e.target.style.display = 'none'} className={`absolute -bottom-2 -right-2 w-8 h-8 object-contain z-40 ${eq.badge.cssClass || ''}`} style={{ pointerEvents: 'none' }} />
            )}
          ...
          <div className="flex-1 text-center md:text-left mt-4 md:mt-0">
            ...
            <div className="w-full max-w-md mx-auto md:mx-0 bg-black/40 p-5 rounded-2xl border border-white/5 mt-4 backdrop-blur-md relative overflow-hidden shadow-2xl">
              ...
              <div className="relative w-full h-4 bg-[#0a0d14] rounded-full overflow-hidden border border-white/5 shadow-inner z-10">
          ...
          <div className="flex gap-2">
            <button onClick={() => setIsEditing(!isEditing)} className="bg-black/50 text-blue-200 px-5 py-3 rounded-xl text-xs uppercase tracking-widest font-black flex items-center gap-2 transition-all duration-300 hover:bg-blue-950/30 hover:text-white border border-blue-900/30 shadow-sm"><Edit3 className="w-4 h-4" /> {isEditing ? 'Cancelar' : 'Editar'}</button>
        ...
        {isEditing ? (
          <form onSubmit={handleSave} className="bg-[#0a0d14]/80 border border-blue-900/30 rounded-2xl p-6 sm:p-8 animate-in slide-in-from-bottom-4 shadow-[0_0_30px_rgba(37,99,235,0.05)] backdrop-blur-md relative overflow-hidden">
            ...
            <div className="space-y-5 relative z-10">
              <div>
                 ...
                 <input type="text" value={name} onChange={e => setName(e.target.value)} className="w-full bg-[#07090e] border border-blue-900/30 rounded-xl px-4 py-3.5 text-white text-sm font-medium outline-none focus:border-blue-500/50 transition-colors duration-300 shadow-inner" placeholder="Ex: Mestre Cósmico"/>
              </div>
              <div>
                 <label className="block text-[10px] font-black text-blue-400/70 uppercase tracking-widest mb-2">Marca na Alma (Biografia)</label>
                 <textarea value={bio} onChange={e => setBio(e.target.value)} rows={3} className="w-full bg-[#07090e] border border-blue-900/30 rounded-xl px-4 py-3.5 text-white text-sm resize-none outline-none focus:border-blue-500/50 transition-colors duration-300 shadow-inner" placeholder="Deixe sua marca no Abismo..."></textarea>
              </div>
            </div>
            ...
            {activeTab === "Estatisticas" && (
              <div className="space-y-4 animate-in fade-in duration-300">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="bg-black/30 border border-blue-900/20 p-5 rounded-2xl text-center shadow-inner"><div className="text-3xl font-black text-blue-50 mb-2 tracking-tighter">{!dataLoaded ? <Loader2 className="w-6 h-6 animate-spin mx-auto text-blue-500"/> : Object.keys(libraryData).length}</div><div className="text-[9px] text-blue-400/60 uppercase font-black tracking-widest">Salvos</div></div>
                  <div className="bg-black/30 border border-blue-900/20 p-5 rounded-2xl text-center shadow-inner"><div className="text-3xl font-black text-blue-500 mb-2 tracking-tighter">{!dataLoaded ? <Loader2 className="w-6 h-6 animate-spin mx-auto text-blue-500"/> : historyData.length}</div><div className="text-[9px] text-blue-400/60 uppercase font-black tracking-widest">Lidos</div></div>
                  <div className="bg-black/30 border border-blue-900/20 p-5 rounded-2xl text-center shadow-inner"><div className="text-3xl font-black text-amber-500 mb-2 tracking-tighter">{!dataLoaded ? <Loader2 className="w-6 h-6 animate-spin mx-auto text-amber-500"/> : obrasLidasIds.length}</div><div className="text-[9px] text-amber-400/60 uppercase font-black tracking-widest">Iniciadas</div></div>
                  <div className="bg-black/30 border border-blue-900/20 p-5 rounded-2xl text-center shadow-inner"><div className="text-3xl font-black text-red-500 mb-2 tracking-tighter">{!dataLoaded ? <Loader2 className="w-6 h-6 animate-spin mx-auto text-red-500"/> : Object.values(libraryData).filter(s=>s==='Favoritos').length}</div><div className="text-[9px] text-red-400/60 uppercase font-black tracking-widest">Favoritos</div></div>
                </div>
              </div>
            )}
            ...
            {activeTab === "Historico" && (
                <div className="animate-in fade-in duration-300">
                    {historyData.length === 0 ? (
                        <div className="text-center py-12 bg-black/30 rounded-2xl border border-blue-900/20 shadow-inner"><History className="w-10 h-10 mx-auto text-blue-900/40 mb-3"/><p className="text-blue-800/60 font-black text-[10px] uppercase tracking-widest">Nenhum rastro detectado no Vazio.</p></div>
                    ) : (
                       <div className="flex flex-col gap-3">
                          {historyData.slice(0, 15).map(hist => {
                              const mg = mangas.find(m => m.id === hist.mangaId);
                              return (
                                  <div key={hist.id} onClick={() => { if(mg) onNavigate('details', mg); }} className="bg-black/30 border border-blue-900/20 p-3 rounded-2xl flex items-center gap-4 cursor-pointer hover:border-blue-500/40 transition-colors duration-300 shadow-sm group">
