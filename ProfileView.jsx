import React, { useState, useEffect, useRef } from 'react'; import { Compass, History, Library, Camera, Edit3, LogOut, Loader2, BookOpen, AlertTriangle, Trophy, Zap, Trash2, RefreshCw, Settings, Flame, Eye, Bookmark, Hexagon, Crown, Ghost, Lock, Sparkles, Box, ChevronRight, Swords, Monitor, Bell, EyeOff, Plus } from 'lucide-react'; import { updateProfile } from "firebase/auth"; import { doc, setDoc, deleteDoc, updateDoc, getDocs, collectionGroup, query } from "firebase/firestore"; import { auth, db } from './firebase'; import { APP_ID } from './constants'; import { compressImage, getLevelRequirement, getLevelTitle, cleanCosmeticUrl, timeAgo, getRarityColor } from './helpers';

// CARTÃO DE ESTATÍSTICA DETALHADA const StatCard = ({ icon, label, value, colorClass }) => ( <div className={bg-[#050505] border border-white/5 rounded-2xl p-5 flex flex-col items-center justify-center text-center shadow-lg hover:border-${colorClass.split('-')[1]}/50 transition-colors group}> <div className={p-3 bg-[#0a0a0c] rounded-full border border-white/5 ${colorClass} mb-4 group-hover:scale-110 transition-transform duration-300}> <StatIcon icon={icon} className="w-6 h-6" /> </div> <span className="text-xl sm:text-2xl font-black text-white">{value}</span> <span className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mt-1.5">{label}</span> <span className="text-[8px] text-gray-700 font-medium mt-0.5">{label === 'Obras Salvas' ? 'Total de mangás' : label === 'Caps Lidos' ? 'Capítulos lidos' : label === 'Iniciadas' ? 'Mangás começados' : 'Poder Vital total'}</span> </div> );

// ÍCONE DE ESTATÍSTICA FLEXÍVEL const StatIcon = ({ icon: Icon, className }) => <Icon className={className} />;

// CARTÃO SOMBRIO KAGE (Para o cabeçalho do perfil) const ShadowCard = ({ children, className = "" }) => ( <div className={bg-[#0a0a0c]/90 border border-red-600/30 rounded-3xl p-6 shadow-[0_0_40px_rgba(220,38,38,0.1)] relative overflow-hidden backdrop-blur-xl ${className}}> <div className="absolute top-0 left-1/2 -translate-x-1/2 w-1/2 h-[1px] bg-gradient-to-r from-transparent via-red-600/60 to-transparent"></div> <div className="relative z-10">{children}</div> </div> );

// EMBLEMA DE CONQUISTA const AchievementBadge = ({ badge, isEquipped, onEquip }) => ( <div className={flex items-center p-3 sm:p-4 border transition-all duration-300 relative group rounded-2xl overflow-hidden ${badge.condition ? isEquipped ? 'bg-[#0a0a0c] border-red-500 shadow-[0_0_15px_rgba(220,38,38,0.2)]' : 'bg-[#050505] border-white/5 hover:border-red-600/50' : 'bg-[#030305] border-white/5 opacity-40 grayscale'}}> <div className={w-14 h-14 sm:w-16 sm:h-16 flex-shrink-0 rounded-full flex items-center justify-center mr-4 transition-transform duration-500 group-hover:scale-105}> {badge.image ? <img src={badge.image} alt={badge.title} className="w-full h-full object-contain" /> : <StatIcon icon={badge.icon} className={w-6 h-6 ${badge.condition ? badge.colorClass : 'text-gray-600'}} />} </div> <div className="flex-1 min-w-0"> <h4 className={font-black text-[11px] sm:text-xs uppercase tracking-widest line-clamp-1 mb-1 ${badge.condition ? (isEquipped ? 'text-red-400' : 'text-white') : 'text-gray-500'}}>{badge.title}</h4> <p className="text-[9px] sm:text-[10px] text-gray-400 font-bold line-clamp-2 leading-snug">{badge.description}</p> {badge.condition && <button onClick={() => onEquip(badge)} className={mt-2 text-[9px] font-black uppercase tracking-widest transition-colors ${isEquipped ? 'text-red-500' : 'text-gray-500 hover:text-white'}}>{isEquipped ? '✓ Equipado' : 'Equipar'}</button>} </div> </div> );

export function ProfileView({ user, userProfileData, historyData, libraryData, dataLoaded, userSettings, updateSettings, onLogout, onUpdateData, showToast, mangas, onNavigate, shopItems = [] }) { const [isEditing, setIsEditing] = useState(false); const [activeTab, setActiveTab] = useState("Estatísticas"); const [inventoryCategory, setInventoryCategory] = useState("avatar"); const [name, setName] = useState(''); const [bio, setBio] = useState(''); const [avatarBase64, setAvatarBase64] = useState(''); const [coverBase64, setCoverBase64] = useState(''); const [loading, setLoading] = useState(false); const [confirmAction, setConfirmAction] = useState(null); const [rankingList, setRankingList] = useState([]); const [loadingRank, setLoadingRank] = useState(false);
