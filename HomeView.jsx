...
export function HomeView({ mangas, onNavigate, dataSaver }) {
    ...
    return (
        <div className="pb-24 animate-in fade-in duration-700 bg-[#030712] relative overflow-hidden">
            {/* Fundos leves, sem animação pesada, apenas aura cósmica */}
            <div className="absolute top-0 left-0 w-[500px] h-[500px] bg-blue-600/10 rounded-full blur-[120px] pointer-events-none mix-blend-screen"></div>
            <div className="absolute top-[40%] right-0 w-[400px] h-[400px] bg-indigo-600/10 rounded-full blur-[120px] pointer-events-none mix-blend-screen"></div>

            {destaques.length > 0 && (
                <div className="relative w-full h-[55vh] md:h-[70vh] max-h-[700px] overflow-hidden mb-12 group bg-[#030712]">
                        ...
                        <div key={manga.id} className={`absolute inset-0 w-full h-full transition-all duration-[1200ms] ease-in-out ${index === currentSlide ? 'opacity-100 scale-100 z-10' : 'opacity-0 scale-105 z-0'}`}>
                            ...
                            <div className="absolute bottom-0 left-0 w-full p-6 md:p-12 max-w-7xl mx-auto flex items-end gap-8 md:gap-12">
                                <div className="hidden md:block w-44 md:w-56 relative group-hover:-translate-y-2 transition-transform duration-700">
                                    <div className="absolute -inset-2 bg-gradient-to-r from-blue-500 to-indigo-500 rounded-2xl blur-xl opacity-30 group-hover:opacity-60 transition-opacity duration-500"></div>
                                ...
                                <div className="flex-1 pb-4 md:pb-8 relative z-20">
                                    <div className="flex items-center gap-3 mb-4">
                                        <span className="bg-transparent border border-blue-400 text-blue-400 text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-[0.2em] shadow-[0_0_10px_rgba(96,165,250,0.3)]">Destaque Astral</span>
            ...
            <div className="mt-8 md:mt-12 px-4 md:px-8 max-w-7xl mx-auto relative z-10">
                <div className="flex items-center justify-between mb-8">
                    <h2 className="text-2xl font-black text-white flex items-center gap-3 tracking-tight">
                        <Flame className="w-6 h-6 text-indigo-400" /> Em Alta
                    </h2>
                    <button onClick={() => onNavigate('popular')} className="text-[10px] md:text-xs font-black text-indigo-300 hover:text-white uppercase tracking-widest flex items-center transition-colors bg-indigo-500/10 px-4 py-2 rounded-full border border-indigo-500/20 hover:bg-indigo-500/20">
                ...
                <div className="flex overflow-x-auto gap-5 pb-8 snap-x snap-mandatory no-scrollbar items-stretch" style={{ WebkitOverflowScrolling: 'touch' }}>
                    {populares.map(manga => (
                        <div key={manga.id} onClick={() => onNavigate('details', manga)} className="flex-none w-[130px] sm:w-[150px] md:w-[180px] snap-start cursor-pointer group">
                            <div className={`relative aspect-[2/3] rounded-[2rem] overflow-hidden bg-[#030712] border border-white/5 group-hover:border-indigo-400/50 shadow-lg group-hover:shadow-[0_0_25px_rgba(99,102,241,0.3)] transition-all duration-500 ${dataSaver ? 'blur-[2px]' : ''}`}>
                                ...
                                <div className="absolute inset-0 bg-gradient-to-t from-[#030712] via-transparent to-transparent opacity-60 group-hover:opacity-90 transition-opacity duration-500"></div>
                            </div>
                            <h3 className="text-sm font-bold text-gray-300 mt-4 line-clamp-1 group-hover:text-indigo-300 transition-colors text-center px-2">{manga.title}</h3>
                        </div>
                    ))}
                </div>
            </div>
            ...
            <div className="mt-12 px-4 md:px-8 max-w-7xl mx-auto relative z-10">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
                    <h2 className="text-2xl font-black text-white flex items-center gap-3 tracking-tight">
                        <Sparkles className="w-6 h-6 text-blue-400" /> Novidades
                    </h2>
                    
                    <div className="w-full sm:w-auto overflow-x-auto no-scrollbar">
                        <div className="flex items-center gap-2 pb-2 px-1 w-max">
                            <ListFilter className="w-4 h-4 text-blue-500 flex-shrink-0 mr-2" />
                            {filterOptions.map(opt => (
                                <button key={opt} onClick={() => handleFilterChange(opt)} className={`px-5 py-2 rounded-full text-[10px] font-black uppercase tracking-[0.1em] whitespace-nowrap transition-all duration-300 ${filter === opt ? 'bg-blue-600 text-white shadow-[0_0_15px_rgba(37,99,235,0.4)]' : 'bg-transparent text-gray-400 hover:text-white hover:bg-white/10'}`}>
                                    {opt}
                                </button>
                            ))}
                        </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-5 md:gap-6">
                    {currentItems.length > 0 ? currentItems.map(manga => (
                        <div key={manga.id} onClick={() => onNavigate('details', manga)} className="cursor-pointer group flex flex-col gap-2">
                            <div className={`relative aspect-[2/3] rounded-3xl overflow-hidden bg-[#030712] border border-white/5 group-hover:border-blue-400/50 shadow-md group-hover:shadow-[0_0_20px_rgba(96,165,250,0.2)] transition-all duration-500 ${dataSaver ? 'blur-[1px]' : ''}`}>
                                ...
                                {manga.chapters && manga.chapters.length > 0 && (
                                    <div className="absolute top-3 left-3 bg-[#030712]/80 backdrop-blur-md border border-blue-500/30 text-blue-300 text-[9px] font-black px-2.5 py-1 rounded-lg uppercase tracking-widest shadow-lg">
                                        Cap {manga.chapters[0].number}
                                    </div>
                                )}
                                <div className="absolute bottom-0 left-0 w-full bg-gradient-to-t from-[#030712] via-[#030712]/80 to-transparent p-3 pt-10">
                                    <p className="text-[9px] text-blue-400 font-bold uppercase tracking-widest drop-shadow-md">{timeAgo(manga.createdAt)}</p>
                                </div>
                            </div>
                            <h3 className="font-bold text-sm text-gray-300 line-clamp-2 leading-snug group-hover:text-blue-400 transition-colors duration-300 px-2 mt-1">{manga.title}</h3>
                        </div>
                    )) : (
                        <div className="col-span-full py-20 flex flex-col items-center justify-center bg-white/[0.02] rounded-[3rem] border border-white/5 backdrop-blur-sm">
                            <BookmarkPlus className="w-12 h-12 text-blue-900 mx-auto mb-4" />
                ...
                {totalPages > 1 && (
                    <div className="mt-16 flex items-center justify-center gap-3">
                        <button disabled={currentPage === 1} onClick={() => setCurrentPage(prev => prev - 1)} className="p-3 rounded-full bg-white/[0.03] border border-white/5 text-gray-400 disabled:opacity-20 hover:bg-blue-500/20 hover:text-blue-400 hover:border-blue-500/30 transition-all">
                            <ChevronLeft className="w-5 h-5" />
                        </button>
                        <div className="flex items-center gap-2 bg-white/[0.02] p-1.5 rounded-full border border-white/5 backdrop-blur-md">
                            {[...Array(totalPages)].map((_, i) => (
                                <button key={i} onClick={() => setCurrentPage(i + 1)} className={`w-10 h-10 rounded-full font-black text-xs transition-all duration-300 ${currentPage === i + 1 ? 'bg-blue-600 text-white shadow-[0_0_15px_rgba(37,99,235,0.5)]' : 'bg-transparent text-gray-500 hover:text-white hover:bg-white/10'}`}>
                                    {i + 1}
                                </button>
                            ))}
                        </div>
                        <button disabled={currentPage === totalPages} onClick={() => setCurrentPage(prev => prev + 1)} className="p-3 rounded-full bg-white/[0.03] border border-white/5 text-gray-400 disabled:opacity-20 hover:bg-blue-500/20 hover:text-blue-400 hover:border-blue-500/30 transition-all">
