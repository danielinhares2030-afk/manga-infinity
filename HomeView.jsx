...
export function HomeView({ mangas, onNavigate, dataSaver }) {
    ...
    return (
        <div className="pb-24 animate-in fade-in duration-500 bg-[#0f1117] min-h-screen relative font-sans text-slate-200">
            {/* Fundo Leve (Sem blur pesado, apenas um gradiente suave de portal/aventura no topo) */}
            <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-sky-900/20 via-[#0f1117] to-[#0f1117] pointer-events-none z-0"></div>

            {/* DESTAQUES (SLIDER ÉPICO - ESTILO GUILDA) */}
            {destaques.length > 0 && (
                <div className="relative w-full h-[55vh] md:h-[70vh] max-h-[700px] overflow-hidden mb-12 group bg-[#0f1117] z-10 border-b-4 border-sky-500/20 shadow-lg">
                    {destaques.map((manga, index) => (
                        <div key={manga.id} className={`absolute inset-0 w-full h-full transition-opacity duration-[800ms] ease-in-out ${index === currentSlide ? 'opacity-100 z-10' : 'opacity-0 z-0'}`}>
                            <div className="absolute inset-0 bg-[#0f1117]">
                                <img src={manga.coverUrl} className={`w-full h-full object-cover opacity-30 ${dataSaver ? 'blur-sm' : ''}`} alt="Background" loading="lazy" />
                            </div>
                            ...
                            <div className="absolute bottom-0 left-0 w-full p-6 md:p-12 max-w-7xl mx-auto flex items-end gap-6 md:gap-12">
                                <div className="hidden md:block w-40 md:w-56 relative group-hover:-translate-y-2 transition-transform duration-500">
                                    <div className="absolute -inset-1 bg-gradient-to-b from-amber-400 to-orange-600 rounded-xl blur opacity-50"></div>
                                    <img src={manga.coverUrl} className={`relative rounded-xl shadow-2xl border-2 border-[#1a1f2e] z-10 ${dataSaver ? 'blur-[1px]' : ''}`} alt="Capa" loading="lazy" />
                                </div>
                    ...
                    <div className="absolute bottom-6 right-6 md:bottom-8 md:right-12 z-20 flex gap-2">
                        {destaques.map((_, i) => (
                            <button key={i} onClick={() => setCurrentSlide(i)} className={`h-2 rounded-full transition-all duration-300 ${i === currentSlide ? 'w-8 bg-amber-400' : 'w-2 bg-slate-600 hover:bg-slate-400'}`} />
                        ))}
                    </div>
                </div>
            )}

            {/* SESSÃO: POPULARES (Ação) */}
                ...
                <div className="flex overflow-x-auto gap-4 md:gap-5 pb-6 snap-x snap-mandatory no-scrollbar items-stretch" style={{ WebkitOverflowScrolling: 'touch' }}>
                    {populares.map(manga => (
                        <div key={manga.id} onClick={() => onNavigate('details', manga)} className="flex-none w-[130px] sm:w-[150px] md:w-[170px] snap-start cursor-pointer group">
                            <div className={`relative aspect-[2/3] rounded-xl overflow-hidden bg-[#1a1f2e] border-2 border-transparent group-hover:border-orange-500/50 shadow-md group-hover:shadow-[0_8px_20px_rgba(249,115,22,0.2)] transition-all duration-300 group-hover:-translate-y-1 ${dataSaver ? 'blur-[1px]' : ''}`}>
                                <img src={manga.coverUrl} alt={manga.title} className="w-full h-full object-cover" loading="lazy" />
                                <div className="absolute top-2 right-2 bg-[#0f1117]/90 px-2 py-1 rounded-md border border-orange-500/30 flex items-center gap-1 shadow-sm">
                                    <Star className="w-3 h-3 text-amber-400 fill-amber-400" />
                                    <span className="text-[10px] font-black text-white">{manga.rating ? Number(manga.rating).toFixed(1) : "5.0"}</span>
                                </div>
                                <div className="absolute inset-0 bg-gradient-to-t from-[#0f1117] via-transparent to-transparent opacity-80 group-hover:opacity-50 transition-opacity duration-300"></div>
                            </div>
                            <h3 className="text-sm font-bold text-slate-200 mt-3 line-clamp-1 group-hover:text-orange-400 transition-colors text-center">{manga.title}</h3>
                        </div>
                    ))}
                </div>
            </div>

            {/* SESSÃO: NOVIDADES (Descobertas) */}
            <div className="mt-8 md:mt-12 px-4 md:px-8 max-w-7xl mx-auto relative z-10">
                    ...
                    <div className="w-full sm:w-auto overflow-x-auto no-scrollbar">
                        <div className="flex items-center gap-2 pb-2 px-1 w-max">
                            ...
                            {filterOptions.map(opt => (
                                <button key={opt} onClick={() => handleFilterChange(opt)} className={`px-5 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest whitespace-nowrap transition-all duration-200 ${filter === opt ? 'bg-emerald-500 text-[#0f1117] shadow-md border-b-2 border-emerald-700 translate-y-0' : 'bg-[#1a1f2e] text-slate-400 hover:text-white hover:bg-[#252b3d] border-b-2 border-[#0f1117] hover:-translate-y-0.5'}`}>
                                    {opt}
                                </button>
                            ))}
                        </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 md:gap-5">
                    {currentItems.length > 0 ? currentItems.map(manga => (
                        <div key={manga.id} onClick={() => onNavigate('details', manga)} className="cursor-pointer group flex flex-col gap-2">
                            <div className={`relative aspect-[2/3] rounded-xl overflow-hidden bg-[#1a1f2e] border-2 border-transparent group-hover:border-emerald-500/50 shadow-sm group-hover:shadow-lg transition-all duration-300 group-hover:-translate-y-1 ${dataSaver ? 'blur-[1px]' : ''}`}>
                                <img src={manga.coverUrl} className="w-full h-full object-cover" loading="lazy" />
                                
                                {manga.chapters && manga.chapters.length > 0 && (
                                    <div className="absolute top-2 left-2 bg-[#0f1117]/90 border border-emerald-500/30 text-emerald-400 text-[9px] font-black px-2 py-1 rounded-md uppercase tracking-widest shadow-sm">
                                        Cap {manga.chapters[0].number}
                                    </div>
                                )}
                                
                                <div className="absolute bottom-0 left-0 w-full bg-gradient-to-t from-[#0f1117] via-[#0f1117]/80 to-transparent p-3 pt-8">
                            ...
                            <h3 className="font-bold text-sm text-slate-200 line-clamp-2 leading-snug group-hover:text-emerald-400 transition-colors duration-200 mt-1">{manga.title}</h3>
                        </div>
                    )) : (
                        <div className="col-span-full py-16 flex flex-col items-center justify-center bg-[#1a1f2e]/50 rounded-2xl border border-dashed border-slate-700">
                            <BookmarkPlus className="w-10 h-10 text-slate-600 mx-auto mb-3" />
                            <p className="text-slate-400 font-bold text-xs uppercase tracking-widest">Nenhum tesouro encontrado aqui.</p>
                        </div>
                    )}
                </div>

                {/* PAGINAÇÃO (Pílulas divertidas e leves) */}
                {totalPages > 1 && (
                    <div className="mt-12 flex items-center justify-center gap-3">
                        <button disabled={currentPage === 1} onClick={() => setCurrentPage(prev => prev - 1)} className="p-3 rounded-lg bg-[#1a1f2e] text-slate-400 disabled:opacity-30 hover:bg-sky-500 hover:text-white transition-all active:scale-95 shadow-sm">
                            <ChevronLeft className="w-5 h-5" />
                        </button>
                        
                        <div className="flex items-center gap-2 bg-[#1a1f2e] p-1.5 rounded-lg shadow-sm">
                            {[...Array(totalPages)].map((_, i) => {
                                // Lógica para não poluir telas pequenas com 50 páginas
                                if (totalPages > 5 && (i + 1 !== 1 && i + 1 !== totalPages && Math.abs(currentPage - (i + 1)) > 1)) {
                                    if (i + 1 === 2 || i + 1 === totalPages - 1) return <span key={i} className="text-slate-500 px-1 font-black">...</span>;
                                    return null;
                                }
                                return (
                                    <button key={i} onClick={() => setCurrentPage(i + 1)} className={`w-10 h-10 rounded-md font-black text-sm transition-all duration-200 ${currentPage === i + 1 ? 'bg-sky-500 text-white shadow-md' : 'bg-transparent text-slate-400 hover:bg-slate-700 hover:text-white'}`}>
                                        {i + 1}
                                    </button>
                                );
                            })}
                        ...
                        <button disabled={currentPage === totalPages} onClick={() => setCurrentPage(prev => prev + 1)} className="p-3 rounded-lg bg-[#1a1f2e] text-slate-400 disabled:opacity-30 hover:bg-sky-500 hover:text-white transition-all active:scale-95 shadow-sm">
