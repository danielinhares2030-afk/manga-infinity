...
import { Star, Clock, ListFilter, ChevronRight, Play, ChevronLeft, Moon, Flame } from 'lucide-react';
...
export function HomeView({ mangas, onNavigate, dataSaver }) {
    ...
    return (
        <div className="pb-24 animate-in fade-in duration-300 bg-[#050505] min-h-screen relative font-sans text-white overflow-x-hidden">
            ...
            {/* Fundo Ambiente Suave (Sem pesar a interface) */}
            <div className="absolute top-0 left-0 w-full h-[400px] bg-gradient-to-b from-red-900/10 to-transparent pointer-events-none z-0"></div>

            {/* SESSÃO: DESTAQUES (CARROSSEL MENOR E MAIS LEVE) */}
            {destaques.length > 0 && (
                    ...
                    {destaques.map((manga, index) => (
                        <div key={manga.id} className={`absolute inset-0 w-full h-full transition-opacity duration-700 ease-in-out ${index === currentSlide ? 'opacity-100 z-10' : 'opacity-0 z-0'}`}>
                            ...
                            <div className="absolute inset-0 p-6 md:p-10 max-w-7xl mx-auto flex items-center gap-8">
                                ...
                                {/* Capa Flutuante */}
                                <div className="hidden md:block w-40 lg:w-48 flex-shrink-0 cursor-pointer shadow-2xl transition-transform hover:-translate-y-1" onClick={() => onNavigate('details', manga)}>
                    ...
                    {populares.map((manga, index) => (
                        <div key={manga.id} onClick={() => onNavigate('details', manga)} className="flex-none w-[120px] sm:w-[140px] snap-start cursor-pointer group">
                            <div className={`relative aspect-[2/3] rounded-xl overflow-hidden bg-[#0A0A0A] border border-white/5 transition-colors group-hover:border-red-500/50 ${dataSaver ? 'blur-[1px]' : ''}`}>
                                <img src={manga.coverUrl} alt={manga.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" onError={(e) => e.target.src = `https://placehold.co/300x450/0A0A0A/dc2626?text=Oculto`} />
                                
                                <div className="absolute top-2 left-2 w-6 h-6 rounded bg-black/80 backdrop-blur-sm border border-white/10 flex items-center justify-center">
                                    <span className="text-[9px] font-black text-white">#{index + 1}</span>
                                </div>

                                <div className="absolute top-2 right-2 bg-black/80 backdrop-blur-sm px-1.5 py-0.5 rounded border border-white/10 flex items-center gap-1">
            ...
            {/* SESSÃO: CATÁLOGO VIEW LIMPÃO E ORGANIZADO */}
            <div className="px-4 md:px-8 max-w-7xl mx-auto relative z-10">
                    ...
                    {currentItems.length > 0 ? currentItems.map(manga => (
                        <div key={manga.id} onClick={() => onNavigate('details', manga)} className="cursor-pointer group flex flex-col">
                            <div className={`relative aspect-[2/3] rounded-xl overflow-hidden bg-[#0A0A0A] border border-white/5 transition-colors group-hover:border-white/20 ${dataSaver ? 'blur-[1px]' : ''}`}>
                                <img src={manga.coverUrl} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" onError={(e) => e.target.src = `https://placehold.co/300x450/050505/3b82f6?text=Oculto`} />
                                
                                {manga.chapters && manga.chapters.length > 0 && (
                                    <div className="absolute top-2 left-2 bg-black/80 backdrop-blur-sm border border-white/10 text-white text-[9px] font-black px-2 py-0.5 rounded uppercase tracking-widest">
                ...
                {/* Paginação Discreta e Funcional */}
                {totalPages > 1 && (
